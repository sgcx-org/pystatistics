...
def cmd_bump(new_version: str) -> None:
    ...
    print("  4. Stage EVERY change that belongs in this release — a commit ships the")
    print("     index, not the working tree, so anything left unstaged is silently")
    print("     dropped from the release. The version/doc files below are the minimum;")
    print("     add your source changes too:")
    print(f"       git add pyproject.toml {package}/__init__.py CHANGELOG.md README.md")
    print("       git add <source files changed for this release>")
    print("     Then confirm nothing release-relevant is left behind:")
    print("       git diff --name-only    # must be empty (or only unrelated WIP)")
